...
public class MMXAuthHeadersFilter implements ContainerRequestFilter {
  private static final Logger LOGGER = LoggerFactory
      .getLogger(MMXAuthHeadersFilter.class);

  @Override
  public void filter(ContainerRequestContext containerRequestContext)
      throws IOException {
    String appId = containerRequestContext.getHeaders().getFirst(
        MMXServerConstants.HTTP_HEADER_APP_ID);
    String apiKey = containerRequestContext.getHeaders().getFirst(
        MMXServerConstants.HTTP_HEADER_REST_API_KEY);
    LOGGER.trace("filter : appId={}, apiKey={}", appId, apiKey);

    // AppID is no longer required for token based authentication.
    if (Strings.isNullOrEmpty(appId)) {
      LOGGER.warn("filter : appId is empty");
//      Response response = RestUtils
//          .buildMissingHeaderResponse(MMXServerConstants.HTTP_HEADER_APP_ID);
//      containerRequestContext.abortWith(response);
    ...
    if (Strings.isNullOrEmpty(apiKey)) {
      ...
      Response response = RestUtils
          .buildMissingHeaderResponse(MMXServerConstants.HTTP_HEADER_REST_API_KEY);
      containerRequestContext.abortWith(response);
      return;
    }
    ...
    if (appEntity == null) {
      ...
      Response response = RestUtils.buildInvalidHeaderResponse(
          ErrorCode.AUTH_BAD_APP_ID,
    ...
    if (!appApiKey.equals(apiKey)) {
      ...
      Response response = RestUtils.buildInvalidHeaderResponse(
          ErrorCode.AUTH_APPID_APIKEY_MISMATCH,
    ...
    containerRequestContext.setProperty(
        MMXServerConstants.MMX_APP_ENTITY_PROPERTY, appEntity);
  }
}
